...
export const statePresentation = (state, isLast) => {
 ...
 labelClass: isLast ? "text-blue-600" : "text-slate-400",
 };
};
...
const RequestPathGraph = ({ query, requestId }) => {
 ...
 <span
 className={`h-3 w-3 shrink-0 rounded-full border-2 ${presentation.dotClass} ${isLast ? "ring-4 ring-blue-100 " : ""}`}
 />
 {!isLast ? (
 <span className="h-px w-[132px] bg-slate-200 " />
 ) : null}
 </div>
 ...
 <div
 className={`mt-1 text-xs font-medium ${presentation.labelClass}`}
 >
 {presentation.label}
 </div>
 {state?.description ? (
 <p className="mb-0 mt-1 line-clamp-2 text-xs leading-4 text-slate-400">
 {state.description}
 </p>
 ) : null}
 </div>
