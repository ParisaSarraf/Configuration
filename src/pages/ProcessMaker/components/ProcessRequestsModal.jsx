...
const requestStatePresentation = (stateType) => {
  if (["denied", "rejected", "reject"].includes(stateType))
    return {
      dot: "bg-rose-500",
      pill: "bg-rose-50 text-rose-600 dark:bg-rose-950/30",
    };
  if (["cancelled", "canceled", "cancel"].includes(stateType))
    return {
      dot: "bg-amber-500",
      pill: "bg-amber-50 text-amber-700 dark:bg-amber-950/30",
    };
  if (["complete", "completed", "done"].includes(stateType))
    return {
      dot: "bg-emerald-500",
      pill: "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30",
    };
  if (stateType === "start")
    return {
      dot: "bg-sky-500",
      pill: "bg-sky-50 text-sky-700 dark:bg-sky-950/30",
    };
  return {
    dot: "bg-indigo-500",
    pill: "bg-indigo-50 text-indigo-700 dark:bg-indigo-950/30",
  };
};
...
const ProcessRequestsModal = ({ open, process, onClose, onViewSubmission }) => {
  ...
  const renderTransitions = (record) => {
    ...
    if (!transitions.length)
      return (
        <div className="rounded-xl border border-dashed border-slate-200 px-4 py-6 text-center text-xs text-slate-400 dark:border-slate-700">
              ...
              >
                <div className="min-w-0">
                  <div className="flex min-w-0 items-center gap-2 text-xs">
                    ...
                    <ArrowLeftOutlined className="shrink-0 text-[10px] text-slate-300" />
                    <span className="max-w-40 truncate font-semibold text-slate-800 dark:text-slate-100">
                      {transition.nextStateName}
                    </span>
                  </div>
                  {transition.actionDescription ? (
                    <p className="mb-0 mt-1.5 line-clamp-1 text-[11px] text-slate-400">
                      {transition.actionDescription}
                    </p>
                  ) : null}
                </div>
  ...
  const renderRequest = (record) => {
    ...
    return (
      <article
        key={record.rowKey}
        className="overflow-hidden rounded-2xl border border-slate-200 bg-white transition-colors hover:border-slate-300 dark:border-slate-700 dark:bg-slate-900"
      >
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="m-0 truncate text-sm font-semibold text-slate-800 dark:text-slate-100">
                  {record.title}
                </h3>
                <span
                  className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${state.pill}`}
                >
                  {record.stateName}
                </span>
              </div>
        ...
        {isExpanded ? (
          <div className="border-t border-slate-100 bg-slate-50/60 p-4 dark:border-slate-800 dark:bg-slate-950/30">
