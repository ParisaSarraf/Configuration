...
const ValueBox = ({ children, className = "" }) => (
  <div
    className={`min-h-[42px] w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-100 ${className}`}
  >
    {children}
  </div>
);
...
const StaticBlock = ({ field }) => {
  ...
  return (
    <p className="whitespace-pre-line text-xs leading-6 text-slate-500 dark:text-slate-400">
      {text}
    </p>
  );
};
        ...
        <thead className="bg-slate-100 dark:bg-slate-800">
          <tr>
            {columns.map((column) => (
              <th
                key={column.key}
                className="p-2 text-right font-semibold text-slate-600 dark:text-slate-300"
              >
                {column.label}
              </th>
            ))}
...
const FilledField = ({ field, formData }) => {
  ...
  const renderValue = () => {
    if (!found) {
      return (
        <ValueBox className="text-slate-400 dark:text-slate-500">
          بدون مقدار
        </ValueBox>
      );
    }
    ...
    return (
      <ValueBox
        className={
          isEmptyValue(value) ? "text-slate-400 dark:text-slate-500" : ""
        }
      >
        {formatFieldValue(field, value)}
      </ValueBox>
  ...
  return (
    ...
    >
      <div className="mb-1 flex items-center gap-1 text-xs font-semibold text-slate-600 dark:text-slate-300">
        <span>{label}</span>
  ...
  return (
    <div className="space-y-4" dir="rtl">
      ...
      {allSections.map((section) => (
        ...
        >
          {section.title ? (
            <header className="mb-3 border-b border-dashed border-slate-200 pb-2 dark:border-slate-700">
