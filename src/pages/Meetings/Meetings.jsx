...
import {
  useDeleteMeeting,
  useGetProductMeetings,
} from "@/QueryServises/MeetingQuery/index.js";
...
import MinutesRelatedToActivities from "@/pages/Meetings/components/MinutesRelatedToActivities/MinutesRelatedToActivities.jsx";
...
const Meetings = () => {
  ...
  const { setModal, closeModal, isOpen, modalData, modalMode, modalType } =
    useModal();
  ...
  const {
    ...
    refetch,
    isLoading,
  } = useGetProductMeetings(currentProduct?.id);
  const { mutateAsync: deleteMeeting } = useDeleteMeeting();
  const safeMeetingData = meetingData || [];

  const meetingsWithActivities = safeMeetingData?.filter(
    (item) => item.meeting_activities?.length > 0
  );
    ...
    <Card
      ...
      extra={
        <Button
          ...
          onClick={() =>
            setModal({ mode: "add", data: null, type: "addOrEdirMeeting" })
          }
          icon={<PlusOutlined />}
          title="صورت جلسات"
        />
