    ...
    const handleFinish = useCallback(async (values) => {
        ...
        try {
            const formattedFilters = {
                ...
                states: values.states && values.states.length > 0 ? values.states.join(',') : undefined,
                files_number: values.files_number && values.files_number.length > 0 ? values.files_number.join(',') : undefined,
                with_children: values.with_children,
            };

            Object.keys(formattedFilters).forEach(key => {
                if (formattedFilters[key] === undefined || formattedFilters[key] === '' || formattedFilters[key] === null) {
