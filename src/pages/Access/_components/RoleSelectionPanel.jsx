...
const RoleSelectionPanel = ({selectedUserId, selectedRoleId, onSelectRole, onDeleteAccess}) => {
 ...
 className={`p-3 my-2 rounded-lg border transition-colors ${selectedRoleId === role.id ? 'bg-blue-50 border-blue-300' : 'bg-white border-slate-200'}`}>
 ...
 className={`p-3 my-2 rounded-lg border cursor-pointer transition-colors ${selectedRoleId === role.id ? 'bg-blue-100 border-blue-400' : 'bg-slate-50 hover:bg-slate-100'}`}>
 {role.name}
 </div>
