...
const InfoRow = ({ label, children }) => (
 <div className="grid grid-cols-3 gap-4 py-3 border-b border-slate-200 last:border-b-0">
 <span className="text-sm font-medium text-slate-500 col-span-1">
 {label}
 </span>
 <div className="text-sm text-slate-800 font-medium col-span-2">
 {children || "-"}
 </div>
 </div>
);
...
const SectionTitle = ({ title }) => (
 <h3 className="text-lg font-semibold text-slate-900 mb-3 pt-4 first:pt-0">
 {title}
 </h3>
);
...
const EditionDetailViewModal = ({
 ...
 <InfoRow label="کد کامل">{editionData?.editionData?.full_code}</InfoRow>
 <InfoRow label="نسخه">{editionData?.editionData?.edition}</InfoRow>
 <InfoRow label="دلیل ویرایش">
 {editionData?.editionData?.reasons_editing?.name}
 </InfoRow>
 <InfoRow label="توضیحات نسخه">{editionData?.editionData?.description}</InfoRow>
 </div>
