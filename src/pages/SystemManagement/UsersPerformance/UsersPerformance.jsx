...
const UsersPerformance = () => {
 ...
 render: (_, __, index) => {
 const rank = index + 1;
 const colors = ["#D97706", "#98A2B3", "#D97706"];
 return (
 <div className="flex justify-center">
 {rank <= 3 ? (
 <div className="relative">
 <TrophyOutlined style={{ color: colors[rank - 1], fontSize: '24px' }} />
 <span className="absolute -top-1 -right-1 bg-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center border shadow-sm">
 {rank}
 </span>
 </div>
 ) : (
 ...
 style={{ 
 background: 'linear-gradient(135deg, #315CFF 0%, #2447D8 100%)',
 fontSize: '18px'
 }}
 >
 ...
 <Tag color="gold" className="text-xs m-0 leading-normal border-none">سطح ممتاز</Tag>
 )}
 </div>
 </div>
 </div>
 ...
 <span className={`text-sm font-black ${value > 50 ? 'text-emerald-600' : 'text-red-500'}`}>
 {value}%
 </span>
 {/* <span className="text-xs text-slate-400 uppercase tracking-tighter">Efficiency Score</span> */}
 </div>
 <Progress
 percent={value}
 showInfo={false}
 ...
 return (
 <div className="min-h-screen bg-[#F5F7FA] p-4 sm:p-6 lg:p-8" dir="rtl">
 ...
 className="flex items-center text-slate-600 hover:!text-blue-700 mb-4 font-medium"
 >
 ...
 valueStyle={{ fontSize: '16px', fontWeight: 800, color: '#16A36A' }} 
 />
 <div className="h-12 w-[1px] bg-slate-100 hidden sm:block" />
 {/* <div className="flex flex-col items-center">
 <span className="text-slate-400 text-sm mb-1">توزیع کلی</span>
 <Progress type="dashboard" percent={75} size={60} strokeColor="#315CFF" />
 </div> */}
 </div>
 </Card>
 </Col>
