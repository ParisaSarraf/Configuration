...
import {
  useCreatePrecinctProduct,
  usePrecinctProductList,
  useUpdatePrecinctProduct,
} from "../../../../../QueryServises/precinctQuery";
...
const PrecinctModal = ({
  ...
  const { data: PrecinctList, isFetching: isFetchingPrecinct } =
    usePrecinctProductList();
  const { isPending: isCreating, mutateAsync: createPrecinct } =
    useCreatePrecinctProduct();
  const { isPending: isUpdating, mutateAsync: updatePrecinct } =
    useUpdatePrecinctProduct();
  ...
  useEffect(() => {
    if (modalMode === "edit" && modalData) {
      form.setFieldsValue({
        ...
        life_cycle_id:
          modalData.life_cycle?.id ?? modalData.life_cycle_id ?? undefined,
        ...
        if (item.id !== currentId) {
          ...
          if (item.children && item.children.length > 0) {
            result = result.concat(
              flattenPrecinctList(item.children, currentId)
            );
          }
        }
      });
            ...
            <Col span={6}>
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "لطفاً ترتیب را وارد کنید",
                  },
                ]}
              >
                <Input placeholder="ترتیب" />
              </Form.Item>
            </Col>
            <Col span={18}>
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "لطفاً عنوان حوزه تجارب را وارد کنید",
                  },
                ]}
              >
