...
import RequestOfWarehousePageCol from "@/pages/RequestOfWarehouse/components/RequestOfWarehousePage/RequestOfWarehousePageCol.jsx";
...
const RequestOfWarehousePage = ({
  ...
  const { data: requestOfWareHouseData, refetch: refetchWareHouseData } =
    useGetSupplyListForWareById(
      selectedWareHouseId,
      constructionParam,
      personalityIdsParam,
    );
  const { mutateAsync: createRequestOfWareHouseNumber } =
    useCreateRequestOfWarehouseNumber();
  const { refetch: refetchConfirmed } = useGetConfirmedWarehouseRequestById(
    currentProduct?.id,
  );
  ...
  const handleSend = async () => {
    try {
      ...
      if (payloads.length === 0) {
        message.error(
          "برای ارسال، باید حداقل برای یک محصول تعداد معتبر (بزرگتر از صفر) وارد کنید.",
        );
        return;
      }
